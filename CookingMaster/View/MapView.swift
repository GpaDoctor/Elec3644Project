...
struct MapView: View {
    ...
    @State private var travelTimes: [MKMapItem: TimeInterval] = [:]
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showResultsSheet) {
                SearchResultsView(searchResults: searchResults, travelTimes: travelTimes, userLocation: locationManager.region.center, searchText: searchText, mapSelection: $mapSelection)
    ...
    func searchPlaces() async {
        ...
        // Fetch travel times for the top 5 search results
        let topResults = Array(searchResults.prefix(5))
        for item in topResults {
            let directionsRequest = MKDirections.Request()
            directionsRequest.source = MKMapItem(placemark: .init(coordinate: locationManager.region.center))
            ...
            if let response = try? await directions.calculate(), let route = response.routes.first {
                travelTimes[item] = route.expectedTravelTime
            }
        }
    }
...
struct SearchResultsView: View {
    ...
    var travelTimes: [MKMapItem: TimeInterval]
                ...
                Section(header: Text(searchText).font(.subheadline)) {
                    ForEach(searchResults, id: \.self) { item in
                        VStack(alignment: .leading) {
                            ...
                            if let travelTime = travelTimes[item] {
                                Text(String(format: "Estimated travel time: %.2f minutes", travelTime / 60))
                                    .font(.subheadline)
                            }
                        }
